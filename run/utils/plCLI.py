...
def _get_ckpt(root_dir, v, ckpt_path):
    ...
    # root_dir might be simclr_report/test, we want to replace with v[0] since thats the ckpt we want
    root_dir = root_dir.replace(root_dir.split('/')[-1], v[0])

    # grab version if not given
    if v[1] == -1:
        versions_numbers = sorted(
                                [ int(p.split('/')[-1].removeprefix('version_')) for p in glob.glob(f'{root_dir}/lightning_logs/version_*')]
        ) # a list of versions numebrs [81, 82, 83, 84, 85, 86, 87] that is sorted
        v[1] = versions_numbers[-1]
...
def _changeDefaultRootDir(mode, orig_dir, ckpt_path): 
    ...
    # for test mode, ie we have '../lightning_logs/version_0/checkpoints/epoch=24-step=7175.ckpt',
    # we want to save inside version_0. hence strip the rest and make that the root_dir. only works with version_0/checkpoint/ep .. path style
...
class MyLightningCLI(LightningCLI):
    ...
    def add_arguments_to_parser(self, parser):
        ...
        parser.add_argument(
                            ...
                            default='test' # we want default to be test so we can use it for dm as well and we dont need to se it again, since mode is a source for other link_arguments
                            )
        parser.add_argument(
                            '--ckpt_path', 
                            default='', 
        ...
        parser.add_argument(
                            ...
                            default= None, 
                            type = int,
                            help = 'select seed for reproducability'
                            )
        ...
        # change where we are saving depending on the mode; first arugment is a lis tof arguments to compute_fn
...
class NLPLightningCLI(LightningCLI):
    def add_arguments_to_parser(self, parser):
        ...
        # change mode in data to reflect actual mode -> see if we can reference mode called to replace this;
        # super important since trainer.fit calls datamodule setup, 
        parser.link_arguments("mode" , "data.mode")
        parser.link_arguments("mode" , "model.mode")
        ...
        # link train_testsplit to yaml
        parser.link_arguments("train_test_split" , "data.split")
        
        #####################################################################################################################################
        #                                                       NLP Methods
        #####################################################################################################################################
        parser.link_arguments("data.max_seq_length" , "model.max_seq_len")
        parser.link_arguments("sl_target" , "model.sl_target")
