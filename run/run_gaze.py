...
def fit(cli):
    ...
    if cli.config.mode == 'ssl':
        ...
        if cli.config.ckpt_path: 
            ...
            cli.model.configure_training_mode(
                                            'sl', 
                                            epochs = cli.config.trainer.max_epochs, 
                                            requires_grad = True, 
                                            linear_eval = cli.config.linear_eval,
                                            exclude_list = ['g']
                                            )
...
def test(cli):
    ...
    if cli.config.ckpt_path:
        ...
        if cli.config.load_dm_ckpt: cli.datamodule = cli.datamodule.load_from_checkpoint(cli.config.ckpt_path, num_class=2)
        cli.model.sl_target = cli.config.sl_target
    ...
    acc, confusion_matrix, sens, spec = run_metrics(
                                                    ...
                                                    model_type= 'vit',
                                                    num_classes= cli.config.model.init_args.num_classes
                                                    )
